                ...
                <div style={{
                                ...
                                processMode === ProcessMode.Custom &&  (
                                    <div style={{ marginTop: 10 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 10 }}>
                                            <span style={{ marginRight: 10 }}>段落最大Token：</span>
                                            <Input
                                                value={maxTokensPerParagraph}
                                                onChange={(e: any) => setMaxTokensPerParagraph(Number(e.target.value))}
                                                style={{ width: 200 }}
                                            />
                                        </div>
                                        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 10 }}>
                                            <span style={{ marginRight: 10 }}>每行最大Tokens：</span>
                                            <Input
                                                value={maxTokensPerLine}
                                                onChange={(e: any) => setMaxTokensPerLine(Number(e.target.value))}
                                                style={{ width: 200 }}
                                            />
                                        </div>
                                        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 10 }}>
                                            <span style={{ marginRight: 10 }}>段落之间重叠标记的数目：</span>
                                            <Input
                                                value={overlappingTokens}
                                                onChange={(e: any) => setOverlappingTokens(Number(e.target.value))}
                                                style={{ width: 200 }}
                                            />
                                        </div>
                                    </div>
                    ...
                    }
                    {
                        trainingPattern === TrainingPattern.QA && (
                            <div style={{ marginTop: 10 }}>
                                <span style={{  marginBottom: 10 }}>QA问答模板：</span>
                                <TextArea
                                    value={qAPromptTemplate}
                                    onChange={(v) => setQAPromptTemplate(v.target.value)}
                                    style={{ width: '100%' }}
                                />
                            </div>
                        )
